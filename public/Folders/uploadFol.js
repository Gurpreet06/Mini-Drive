...
//Show Add event template
async function fileUploder() {
    // Stop body to do scroll and add blur affect to background 
    refBody1.style.overflow = 'hidden'
    refcreateBlur1.style.display = 'flex'
    ...
    // Show Animation
    refcreateBlur1.style.opacity = 1
    refcreateEvent1.style.transform = 'translateX(0)'

    // Wait here form animation
    await wait(1)
}

async function fileUploderHide(evt) {
    if (evt.target.getAttribute('id') != 'createBlur1') return
